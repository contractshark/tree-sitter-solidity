...
module.exports = function () {
    return {
      configureWebpack(config, isServer) {
        ...
        if (isServer) {
          rules.push({
            test: /tree-sitter-solidity/,
            loader: "null-loader",
          });
        } else {
          // The way tree-sitter-solidity loads tree-sitter.wasm isn't something that
          // Docusaurus/Webpack identify as an asset. There is currently no way to
          // set location of the file other than patching tree-sitter-solidity.
          ...
          rules.push({
            ...
            options: {
              multiple: [
                ...
                // tree-sitter-solidity will still add a prefix to it unless there is a
        ...
        return {
          // tree-sitter-solidity tries to import "fs", which can be ignored.
